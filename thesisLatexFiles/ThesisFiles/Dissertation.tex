...
It is important to highlight that if a generator is down in previous period, then it may ramp up by <bla> or <bla>, so it can reaches the minimum generation level in the period it is on; otherwise it can not exceed the ramping rate <make it better here>
